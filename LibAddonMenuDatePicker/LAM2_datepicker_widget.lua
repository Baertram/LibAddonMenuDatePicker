...
local translations = {
    ...
    ["fr"] = {
        ...
        MONTHS          =   {"Jan", "Fév", "Mar", "Avr", "Mai", "Jui", "Jui", "Aou", "Sep", "Oct", "Nov", "Déc"},
        MONTHS_LONG     =   {"Janvier", "Février", "Mars", "Avril", "Mai", "Juin", "Juillet", "Août", "Septembre", "Octobre", "Novembre", "Décembre"},
        CURRENT_DAY     =   "Jour courrant",
        CURRENT_MONTH   =   "Mois courrant",
...
local weekDayNames      = translation.WEEKDAYS
local weekDayNamesLong  = translation.WEEKDAYS_LONG
local monthNames        = translation.MONTHS
local monthNamesLong    = translation.MONTHS_LONG
...
--df("buildDateFromData - headline: %s, lang: %s", tostring(data.isHeadline), tostring(lang))
    ...
    if data.isHeadline == true then
        dateStr = data.weekDayNameLong ~= nil and data.weekDayNameLong or data.weekDayName
    else
...
--df("CreateDatePickerLabel - name: %s, onlyUpdate: %s", tostring(labelData.name), tostring(onlyUpdate))
    ...
    if label == nil then
        if onlyUpdate == true then
            df("[LibAddonMenu-2.0 - DatePicker widget]ERROR: Label for update does not exist - name: %s", tostring(labelData.name))
            return
        end
...
function DatePicker:CreateDatePicker()
    ...
    datePickerCurrentYearButton.tooltip = translation.CURRENT_YEAR
    ...
    datePickerMonthMinusButton.tooltip = translation.PREVIOUS_MONTH
    ...
    datePickerMonthPlusButton.tooltip = translation.NEXT_MONTH
    ...
    datePickerCurrentDayButton.tooltip = translation.TODAY
    ...
    for weekDay=1, maxDaysInWeek, 1 do
        ...
        local labelHeadlineData = {
            ...
            weekDayNameLong = weekDayNamesLong[weekDay],
            wrapMode = TEXT_WRAP_MODE_ELLIPSIS,
            hidden = false,
            mouseEnabled = true,
...
local function registerWidget(eventId, addonName)
    if addonName ~= "LibAddonMenuDatePicker" then return end
    em:UnregisterForEvent("LibAddonMenuDatePicker_EVENT_ADD_ON_LOADED", EVENT_ADD_ON_LOADED)

    if not LAM:RegisterWidget("datepicker", widgetVersion) then return end
end

em:RegisterForEvent("LibAddonMenuDatePicker_EVENT_ADD_ON_LOADED", EVENT_ADD_ON_LOADED, registerWidget)
